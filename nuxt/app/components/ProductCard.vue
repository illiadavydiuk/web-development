<template>
  <div v-if="plan" class="w-full max-w-[340px] bg-white rounded-xl overflow-hidden border border-gray-200
    ...
    <div class="p-8 flex flex-col flex-1">
      <h2 class="text-2xl font-bold text-gray-800 mb-6 min-h-[2rem]">{{ plan.name }}</h2>
      ...
      <div class="mb-6">
        ...
        <span class="inline-block mt-2 text-xs font-semibold text-green-700 bg-blue-50 px-2 py-1 rounded">
      ...
      ? 'bg-linear-to-r from-amber-300 to-orange-400 hover:from-yellow-300 hover:to-amber-400 ' +
...
defineProps<{
  plan: {
    ...
    descriptionItems: { mainText: string; subText?: string }[]
  }
}>()
</script>
